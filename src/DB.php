...
class ModelUser extends \RedBeanPHP\SimpleModel
{
    ...
    public function insertNewGame($userName, $hidden_num, $MAX_NUM)
    {
        openDatabase();

        date_default_timezone_set("Europe/Moscow");

        var $gameData = date("d") . "." . date("m") . "." . date("Y");

        var $gameTime = date("H") . ":" . date("i") . ":" . date("s");
    ...
    public function addAttemptInDB($idGame, $proposedNumber, $computerResponds, $numberAttempts)
    {
        openDatabase();
    ...
    public function updateInfoGame($idGame, $gameOutcome)
    {
        openDatabase();
        R::exec("UPDATE gamesInfo SET gameOutcome = '$gameOutcome' WHERE idGame = '$idGame'");
    }

    public function outputListGame($gameOutcome = false)
    {
        openDatabase();
